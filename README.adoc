...
:sectanchors:
:groupId: com.leeharkness
:artifactId: java-archetype
:repository: {artifactId}

A Maven https://maven.apache.org/guides/introduction/introduction-to-archetypes.html[archetype] specifically designed for creating modern Java CLI applications with reasonable defaults and essential dependencies.

== Usage

=== Quick Start (Local Archetype)

If you have this archetype locally (e.g., cloned from Git), use:

* Type `mvn archetype:generate -DarchetypeGroupId=com.leeharkness -DarchetypeArtifactId=java-archetype -DarchetypeVersion=0.0.14-SNAPSHOT`.
...
* `mvn archetype:generate -DarchetypeGroupId=com.leeharkness -DarchetypeArtifactId=java-archetype -DarchetypeVersion=0.0.14-SNAPSHOT -DinteractiveMode=false -DgroupId=mygroupid -DartifactId=myartifactid` also indicates the desired group and artifact ids and removes the prompts.

=== From Eclipse
Eclipse looks at your local archetype catalog. You need to make this archetype appear there.

* Invoke this archetype using one of the commands above. Maven will download it in your local repository (you should now see it under `~/.m2/repository/com/leeharkness/`).
...
* `File` / `New` / `Maven Project`, type `leeharkness` in the filter box. Select this archetype.

Instead of using the Maven local or remote archetypes catalog as indicated above, you may prefer the `Nexus Indexer`: `File` / `New` / `Maven Project`, select `Nexus Indexer` as a catalog, type `leeharkness` in the filter box. Select this archetype. (Tested under Eclipse Oxygen.) (On my install nothing appeared under `Nexus Indexer`, by default. Follow link:Nexus%20Indexer%20from%20Eclipse.adoc[this] to solve that problem.)

== Generated Projects
Projects generated from this archetype have the following characteristics:

* **CLI-focused**: Designed specifically for command-line applications using PicoCLI
* **Modern Java**: Uses Java 17 with recent compiler and surefire plugin versions
* **Shade Plugin**: Configured to create executable JARs with all dependencies
* The generated `pom.xml` includes recent versions of:
** https://picocli.info/[PicoCLI] - Command line interface framework
** https://projectlombok.org/[Lombok] - Reduces boilerplate code with annotations
** https://github.com/google/guava[Guava] - Google's core libraries
** http://www.slf4j.org/[SLF4J] with http://logback.qos.ch/[Logback] - Logging framework
** https://junit.org/junit5/[JUnit Jupiter] - Testing framework
** https://site.mockito.org/[Mockito] - Mocking framework for tests
* The project contains:
** a starter CLI https://github.com/oliviercailloux/java-archetype/blob/master/src/main/resources/archetype-resources/src/main/java/App.java[class] with PicoCLI integration
...
== Best Practices for Using This Archetype

=== Installing the Archetype Locally

To use this archetype as your go-to CLI template, install it locally:

1. Clone this repository: `git clone <your-repo-url>`
2. Install to your local Maven repository: `mvn clean install`
3. The archetype is now available for use with the commands above

=== Creating New CLI Projects

For each new CLI project:

1. Navigate to your projects directory
2. Run: `mvn archetype:generate -DarchetypeGroupId=com.leeharkness -DarchetypeArtifactId=java-archetype -DarchetypeVersion=0.0.14-SNAPSHOT -DinteractiveMode=false -DgroupId=com.yourcompany -DartifactId=your-cli-tool`
3. `cd your-cli-tool`
4. Start coding! The generated project is ready to build and run
...
* Run: `java -jar target/your-cli-tool-1.0.0-SNAPSHOT.jar [args]`
* Run with Maven: `mvn exec:java -Dexec.args="[args]"`

=== Customizing the Archetype

To modify this archetype for your needs:

1. Edit files in `src/main/resources/archetype-resources/`
2. Update version numbers in the generated POM template
3. Add/remove dependencies as needed
4. Reinstall with `mvn clean install`

== How it works

This is a fork of Olivier Cailloux's excellent java-archetype, customized specifically for CLI development. The key differences:

* **PicoCLI Integration**: Pre-configured with PicoCLI for command-line argument parsing
* **Shade Plugin**: Creates executable JARs with all dependencies included
* **Mockito**: Added for comprehensive testing support
* **CLI-focused Examples**: Starter code demonstrates CLI best practices
